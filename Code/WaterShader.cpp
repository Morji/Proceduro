...
bool WaterShader::Initialize(ID3D10Device* device, HWND hwnd)
{
	bool result;


	// Initialize the shader that will be used to draw the model.
	result = InitializeShader(device, hwnd, L"Shaders/watermagic.fx");
	if(!result)	{
		return false;
	}
...
void WaterShader::Render(ID3D10Device* device, int indexCount){
...
bool WaterShader::InitializeShader(ID3D10Device* device, HWND hwnd, WCHAR* filename)
{
	HRESULT result;
	...
	mTechnique = mEffect->GetTechniqueByName("WaterTechnique");
	if(!mTechnique)
	{
		return false;
	}
...
void WaterShader::SetShaderParameters(D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, 
